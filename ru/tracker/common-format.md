...
В запросах к {{ api-short-name }} указывайте заголовки:
 ...
 - `Accept-Language: <тег языка>` - по умолчанию HTTP-запрос возвращает локализованные данные на русском языке, чтобы получить в результате запроса локализованные поля на английском языке укажите тег языка **en**.
